...
train_size1 = int(0.4 * len(dataset))  # 40% for model 1
train_size2 = len(dataset) - train_size1  # Remaining 60% for model 2
train_dataset1, train_dataset2 = random_split(dataset, [train_size1, train_size2])

# Create separate data loaders for each partition
train_loader1 = DataLoader(train_dataset1, batch_size=64, shuffle=True, collate_fn=collate_fn)
...
for epoch in range(num_epochs):
    train(train_loader1, model1, optimizer1, loss_function, device)  # Train model 1 on its partition
    print(f"Epoch {epoch + 1}, Model 1: Finished")
    test(test_loader, model1, loss_function, device)  # Test model 1 on the test set

    train(train_loader2, model2, optimizer2, loss_function, device)  # Train model 2 on its partition
    print(f"Epoch {epoch + 1}, Model 2: Finished")
    test(test_loader, model2, loss_function, device)  # Test model 2 on the test set

# Save Models
torch.save(model1.state_dict(), "model-parallelism-model1.pth")
torch.save(model2.state_dict(), "model-parallelism-model2.pth")
